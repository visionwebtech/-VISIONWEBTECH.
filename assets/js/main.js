...
if (inquiryForm) {
  inquiryForm.addEventListener('submit', (event) => {
    ...
    // Future backend integration can capture and validate this form before WhatsApp redirection.
    const url = `https://wa.me/919546723997?text=${encodeURIComponent(lines.join('\n'))}`;
    window.open(url, '_blank', 'noopener');
  });
}

const heroCanvas = document.getElementById('hero-canvas');
if (heroCanvas && window.matchMedia('(prefers-reduced-motion: no-preference)').matches) {
